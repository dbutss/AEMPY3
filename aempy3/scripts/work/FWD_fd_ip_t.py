...
identstr='_'+aem_system+'_L2D10_t'
nlyr = 3
true_res = ([300.0,10., 1000.0])  #resisitivity p.shape()
...
test_t = ([0.001, 0.01, 0.1, 1., 10., 100.]) 
size_t = np.shape(test_t)

model = model_true
data_obs = nan*np.ones((size_t[0],8))

for ii in range(size_t[0]):
    model[4*nlyr+1]=test_t[ii]
    print(model[4*nlyr:5*nlyr])
