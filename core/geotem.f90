SUBROUTINE aemfwd1d_geotem(mode,alt,nlyr,mvec, calc_data,nspars,spars)
!f2py depend(nlyr) mvec
!f2py depend(nspars) spars
!f2py intent(in) mode,nlyr,js,alt, mvec, nspars, spars
...
!   
    ...
    INTEGER, INTENT(in) :: nlyr, mode, nspars
    ...
    REAL(KIND=8),  INTENT(out), DIMENSION(22) :: calc_data
...
SUBROUTINE aemjac1d_geotem(mode,alt,nlyr,mvec,calc_data,npara,isactive,jacobian,nspars,spars)
!f2py depend(nlyr)  mvec,isactive
!f2py depend(npara) jacobian
!f2py depend(nspars) spars
!f2py intent(in)    nlyr,js,alt,npara,isactive,nspars,spars 
...
!------------------------------------------------------------------------
    ...
    INTEGER, INTENT(in) :: nlyr, npara, mode, nspars
   ...
   ! instrument geometry and constants
   ! not used :offtym = 0.d0, txfreq = 225.d0, 
    REAL(KIND=8) , DIMENSION(nrxf) :: &
    &           zrx(1:nrxf) = 43.d0, xrx(1:nrxf) = 90.d0, yrx(1:nrxf) = 0.d0,  &       
    &           txcln(1:nrxf) = 0.104719758d0 
    REAL(KIND=8) :: txampl =1.d0, pulse =  2.22222228d-03   
    ! txarea =  220.d0       
    ! txampl = 391.d0
    ! pulse = 0.5d0/txfreq
    ! txcln = 6 deg*pi/180
    ...
    ! windows
    REAL(KIND=8), DIMENSION(11) ::  &
    &  topn =  (/ 1.d-6, 0.017d0, 0.035d0, 0.069d0, 0.122d0,   &
    &       0.191d0, 0.295d0, 0.434d0, 0.660d0, 1.007d0,    & 
    &       1.510d0 /),                                     &
    &  tcls =  (/ 0.017d0, 0.035d0, 0.069d0, 0.122d0, 0.191d0, &
    &          0.295d0, 0.434d0, 0.660d0, 1.007d0, 1.510d0, &
    ...
    REAL(KIND=8),  INTENT(in), DIMENSION((7*nlyr)) :: mvec      
    REAL(KIND=8),  INTENT(out), DIMENSION(22,npara) :: jacobian
    
    REAL(KIND=8), ALLOCATABLE :: mcurrent(:)
    
    REAL(KIND=8),  DIMENSION(22) :: calc_data, calc_data1    
...
!     NTYPLS - number of TRP values in 1 PULSE
      ...
      CALL setup_geotem(xrx,yrx,zrx,txcln,prm_td,kppm,ppfac,norm)
     ...
     ! vertical component (fT) 
      ...
      DO ii =1, size(isactive)
         ...
         ! write(*,*) jj
         mcurrent = mvec
         CALL trans_mvec(mode,nlyr,mcurrent)
         ! perturb mvec for paramter ii, to be stored in column jj
         mcurrent(ii) = mcurrent(ii)+dp(ii) 
         CALL untrans_mvec(mode,nlyr,mvec)
         CALL unpack_mvec(nlyr,res,reps,rmu,calf,ctau,cfreq,thk,mcurrent)
         CALL aem1d_td(step,ider,nsx,swx,swy,npuls,pulse,ntypls,  &
     &                     ntyrp,trp,nchnl,topns,tclss,txcln,alt,    &
     &                     zrx,xrx,yrx,                           &
     ...
     ! vertical component (fT) 
         calc_data1(nchnl+1:2*nchnl)  = td_out(1:nchnl,3)*norm(3)
...
END SUBROUTINE aemjac1d_geotem
